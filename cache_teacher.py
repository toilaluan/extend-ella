...
device = "cuda"
text_encoder = T5GemmaEncoder.from_pretrained(
    "Photoroom/prx-1024-t2i-beta", subfolder="text_encoder", dtype=torch.bfloat16
).to(device)
tokenizer = GemmaTokenizerFast.from_pretrained(
    "Photoroom/prx-1024-t2i-beta", subfolder="tokenizer"
)
...
ds = ds.map(
    lambda x: tokenizer(
        x["caption"], max_length=256, padding="max_length", truncation=True
    ),
    num_proc=8,
)
