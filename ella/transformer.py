...
def _compute_default_rope_parameters(
    head_dim: int, base: int = 10000, device="cuda"
...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(self, head_dim: int, device: str = "cuda"):
        super().__init__()
        inv_freq = _compute_default_rope_parameters(
            head_dim=head_dim, base=10000, device=device
        )
...
class Attention(nn.Module):
    def __init__(
        self, hidden_size: int, num_attention_heads: int = 8, head_dim: int = 64
    ):
        super().__init__()
...
class Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.attn(
            norm_hidden_states, attention_mask, position_embeddings
        )
